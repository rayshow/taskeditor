	...
	, InteractorPainting( nullptr )
{
	...
	GLayerDebugOnTopColorMaterial = LandscapeTool::CreateMaterialInstance(LoadObject<UMaterial>(nullptr, TEXT("/Engine/EditorLandscapeResources/LandscapeMaterialTranslucent.LandscapeMaterialTranslucent")));
	ensure(GLayerDebugOnTopColorMaterial);
...
FEdModeLandscape::~FEdModeLandscape()
{
	...
	GLayerDebugOnTopColorMaterial = NULL;
...
void FEdModeLandscape::AddReferencedObjects(FReferenceCollector& Collector)
{
	...
	Collector.AddReferencedObject(GLayerDebugOnTopColorMaterial);
}
...
void FEdModeLandscape::AddLayerInfo(ULandscapeLayerInfoObject* LayerInfo)
{
	if (CurrentToolTarget.LandscapeInfo.IsValid() && CurrentToolTarget.LandscapeInfo->GetLayerInfoIndex(LayerInfo) == INDEX_NONE)
	{
		...
		CurrentToolTarget.LandscapeInfo->MarkPackageDirty();
		UpdateTargetList();
	}
}

bool FEdModeLandscape::ReplaceLayerInfo(FName LayerName, ULandscapeLayerInfoObject* NewLayerInfo)
{
	if (CurrentToolTarget.LandscapeInfo.IsValid() && CurrentToolTarget.LandscapeInfo->GetLayerInfoIndex(NewLayerInfo) != INDEX_NONE)
	{
		ALandscapeProxy* Proxy = CurrentToolTarget.LandscapeInfo->GetLandscapeProxy();
		for (auto& Layer : CurrentToolTarget.LandscapeInfo->Layers) {
			if (Layer.LayerInfoObj == NewLayerInfo) {
