			...
			+ SHorizontalBox::Slot()
				...
				[
					PropertyCustomizationHelpers::MakeAddButton(
						...
						FScopedTransaction Transaction(LOCTEXT("Undo_Create", "Creating New Empty Landscape Layer"));

						FEdModeLandscape* LandscapeEdMode = GetEditorMode();
						
						auto ScapeInfo = LandscapeEdMode->CurrentToolTarget.LandscapeInfo;

						if (ScapeInfo.IsValid())
						{
							//FString Path = ScapeInfo->GetOutermost()->GetName();
							FString Path = ScapeInfo->GetLandscapeProxy()->GetLevel()->GetOutermost()->GetName();
							if (Path.StartsWith("/Temp/"))
							{
								Path = FString("/Game/") + Path.RightChop(FString("/Temp/").Len());
							}
							FString PackageName = Path + FString("_sharedassets/New");
							...
							if (NewLayerDlg->ShowModal() != EAppReturnType::Cancel)
							{
								...
								FName LayerName = FName(*NewLayerDlg->GetAssetName().ToString());

								if (ScapeInfo->GetLayerInfoIndex(LayerName, ScapeInfo->GetLandscapeProxy()) != INDEX_NONE)
								{
									FMessageDialog::Debugf(LOCTEXT("LayerExistsMsg", "Layer Already Exists, Please Input different Layer Name."));
									return;
								}

								UPackage* Package = CreatePackage(NULL, *PackageName);
								ULandscapeLayerInfoObject* LayerInfo = NewObject<ULandscapeLayerInfoObject>(Package, LayerName, RF_Public | RF_Standalone | RF_Transactional);
								...
								LandscapeEdMode->AddShownLayerName(LayerInfo->LayerName);
								LandscapeEdMode->AddLayerInfo(LayerInfo);
							}
						}
					}),TAttribute<FText>(),true )
				]
		];
	}
}
...
void FLandscapeEditorCustomNodeBuilder_TargetLayers::HandleNameCommitted(
	const FText& NewText, ETextCommit::Type CommitInfo,
	TSharedRef<FLandscapeTargetListInfo> TargetInfo)
{
	if (NewText.ToString() != TargetInfo->LayerName.ToString())
	{
		UObject* LayerObj = TargetInfo->LayerInfoObj.Get();
		if (LayerObj) {

			// check weather NewName is exists
			FEdModeLandscape* LandscapeEdMode = GetEditorMode();
			auto ScapeInfo = LandscapeEdMode->CurrentToolTarget.LandscapeInfo;
			FName NewLayerName = FName(*NewText.ToString());
			if (ScapeInfo->GetLayerInfoIndex(NewLayerName, ScapeInfo->GetLandscapeProxy()) != INDEX_NONE)
			{
				FMessageDialog::Debugf(LOCTEXT("LayerNameExistsMsg", "Layer Name Already Exists, Please Input different Layer Name."));
				return;
			}
			...
			//Get Rename Path
			UObject* OldPackage = LayerObj->GetOutermost();
			FString OldPath = LayerObj->GetPathName();
			FString PackagePath = OldPackage->GetPathName();
			int Index = 0;
			PackagePath.FindLastChar(TEXT('/'), Index);
			FString NewPackagePath = PackagePath.Mid(0, Index);
			FString NewName = NewText.ToString();
			check(Index > 0 && NewPackagePath.Len()>0);

			// Rename it to see if it's conflict with exists Object
			TArray<FAssetRenameData> RenameAssets;
			RenameAssets.Add(FAssetRenameData(TargetInfo->LayerInfoObj, NewPackagePath , NewName));
			if (Tools.RenameAssets(RenameAssets)) {
				//Registries.OnAssetRenamed().Broadcast();


				//Rename Successful, notify
				//Registries.AssetRenamed(LayerObj, OldPath);
				FString NewObjPath = NewPackagePath / NewName+"."+ NewName;
				FAssetData AssetData = Registries.GetAssetByObjectPath(FName(*NewObjPath));
				
				if (AssetData.IsValid() && AssetData.GetAsset()->GetClass()->IsChildOf<ULandscapeLayerInfoObject>()) {

					auto NewLayerInfoObj = Cast<ULandscapeLayerInfoObject>(AssetData.GetAsset());
					ALandscapeProxy* Proxy = TargetInfo->Owner.Get();

					// update info in landscape and mode
					FName OldLayerName = TargetInfo->LayerName;
					FString LayerName = TargetInfo->LayerName.ToString();
					bool ret = LandscapeEdMode->ReplaceName(TargetInfo->LayerName, NewLayerName);
					NewLayerInfoObj->LayerName = NewLayerName;
					ret &= LandscapeEdMode->ReplaceLayerInfo(NewLayerName, NewLayerInfoObj);
					check(ret);
					TargetInfo->TargetName = NewText;
					TargetInfo->LayerName = NewLayerName;
					TargetInfo->LandscapeInfo->MarkPackageDirty();
					TargetInfo->LayerInfoObj->MarkPackageDirty();
					TargetInfo->Owner->MarkPackageDirty();

					TArray<UPackage*> PackagesToSave{};
					PackagesToSave.Add(TargetInfo->LayerInfoObj->GetOutermost());
					PackagesToSave.Add(TargetInfo->Owner->GetOutermost());
					PackagesToSave.Add(TargetInfo->LayerInfoObj->GetOutermost());
					
					FEditorFileUtils::PromptForCheckoutAndSave(PackagesToSave, false, false);

					FAssetData OldAssetData = Registries.GetAssetByObjectPath(FName(*OldPath));
					if (OldAssetData.IsValid())
					{
						TArray<UObject*> PackagesToDelete{ OldAssetData.GetAsset() };
				...
				FMessageDialog::Debugf(LOCTEXT("LayerAssetExistsMsg",
					"LayerDataInfo Asset Already Exists, Please Input different Layer Name."));
			}
		}
	}
...
bool FLandscapeEditorCustomNodeBuilder_TargetLayers::VerifyNameChanged(
	const FText& InName, FText& OutError, TSharedRef<FLandscapeTargetListInfo> TargetInfo) const
{
	return true;
}
				...
				[
					...
					[
						...
						[
							...
							.Text_Lambda([Target]() {return Target->TargetName; })
							.Font(IDetailLayoutBuilder::GetDetailFont())
							.HighlightText(FText::FromString("HeightLightText"))
							.OnTextCommitted(this, &FLandscapeEditorCustomNodeBuilder_TargetLayers::HandleNameCommitted, Target)
							.OnVerifyTextChanged(this, &FLandscapeEditorCustomNodeBuilder_TargetLayers::VerifyNameChanged, Target)
							.IsSelected_Lambda([Target]() {return !Target->bValid; })
							.IsReadOnly(false)
						]
						+ SHorizontalBox::Slot()
						.HAlign(HAlign_Right)
					...
					[
						...
						[
							...
							.ObjectPath_Lambda([Target]() {
								return Target->LayerInfoObj != NULL ? Target->LayerInfoObj->GetPathName() : FString();
							})
...
{
	bool bIsDebugLayer = GLandscapeViewMode == ELandscapeViewMode::DebugLayer || GLandscapeViewMode == ELandscapeViewMode::DebugOnTopLayer;
	if ( bIsDebugLayer && Target->TargetType != ELandscapeToolTargetType::Heightmap && Target->LayerInfoObj.IsValid())
	{
		return EVisibility::Visible;
	}
