...
public:
	...
	bool ReplaceLayerInfo(FName LayerName, ULandscapeLayerInfoObject* LayerInfo);
	void AddShownLayerName(FName& Name) { ShownTargetLayerList.Add(Name); }
	bool ReplaceName(FName& OldName, FName& NewName) {
		int Index = ShownTargetLayerList.Find(OldName);
		if (Index != INDEX_NONE) {
			ShownTargetLayerList[Index] = NewName;
			return true;
		}
