...
FDataTableEditor::~FDataTableEditor()
{
	if (ExtToolbarExtender.IsValid())
		GetSharedToolBarExtensibilityManager()->RemoveExtender(ExtToolbarExtender);
...
void FDataTableEditor::HandleConvert2CSV()
{
	auto& Objects = *GetObjectsCurrentlyBeingEdited();
	if (Objects.Num() > 0) {
		UDataTable* DataTable = Cast<UDataTable>(Objects[0]);
		if (DataTable) {
			...
			if (DataTable)
			{
				//if (DataTable->GetPathName().StartsWith(TEXT("/Game")));
				const FText Title = FText::Format(LOCTEXT("DataTable_ExportCSVDialogTitle", "Export '{0}' as CSV..."), FText::FromString(*DataTable->GetName()));
				int Index, Start;
				FString Path = DataTable->GetPathName().Right(DataTable->GetPathName().Len() - 1);
				Path.FindLastChar(TEXT('.'), Index);
				Path.FindChar(TEXT('/'), Start);


				IFileManager& FileManager = IFileManager::Get();
				FString OutFilename = Path.Mid(Start + 1, Index - Start) + TEXT("csv");
				FString AbsPath = FPaths::GameContentDir() + OutFilename;
				if (!AbsPath.IsEmpty())
				{
					FFileHelper::SaveStringToFile(DataTable->GetTableAsCSV(EDataTableExportFlags::UsePrettyPropertyNames | EDataTableExportFlags::UsePrettyEnumNames), *AbsPath);
				}
			}
		}
	}
...
void FDataTableEditor::InitDataTableEditor(const EToolkitMode::Type Mode, const TSharedPtr< class IToolkitHost >& InitToolkitHost, UDataTable* Table)
{
	...
	//add save as csv toolkit button
	TSharedPtr<FUICommandList> Commands = MakeShareable(new FUICommandList);
	Commands->MapAction(DataTableConvertCommand::Get().Convert2CSV,
		FExecuteAction::CreateRaw(this, &FDataTableEditor::HandleConvert2CSV));



	ExtToolbarExtender = MakeShareable(new FExtender);
	ExtToolbarExtender->AddToolBarExtension(TEXT("Asset"), EExtensionHook::After, Commands,
		FToolBarExtensionDelegate::CreateLambda([](FToolBarBuilder& Builder) {
		Builder.AddToolBarButton(DataTableConvertCommand::Get().Convert2CSV,
			NAME_None, TAttribute<FText>(), TAttribute<FText>(), FSlateIcon(FEditorStyle::GetStyleSetName(), "AssetEditor.SaveAsset.Greyscale"));
	}));


	GetSharedToolBarExtensibilityManager()->AddExtender(ExtToolbarExtender);
