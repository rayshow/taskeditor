...
public:
	...
	*/
	...
	ECheckBoxState GetCheckState() const { return State; }
	void SetCheckState(ECheckBoxState InState)
	{
		State = InState;
		for (auto Child : ChildNodes)
		{
			Child->SetCheckState(InState);
		}
		if (ParentNodeWeakPtr.IsValid() && ParentNodeWeakPtr.Pin()->GetParentNode()!=nullptr)
		{
			ParentNodeWeakPtr.Pin()->NotifyChildCheckState(InState);
		}
	}
	void NotifyChildCheckState(ECheckBoxState InState)
	{
		if (InState == ECheckBoxState::Unchecked)
		{
			...
			for (auto Child : ChildNodes)
			{
				if (Child->GetCheckState() == ECheckBoxState::Unchecked)
				{
					AllChecked = false;
				}
			}
